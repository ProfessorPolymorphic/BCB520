...
grade_summary <- grades_long_with_both_legends%>%
  mutate(score = ifelse(Grade == "match", 1, 0))%>%
  group_by(VERAS_Proposal_Number, temp, Field)%>%
  summarize(score=mean(score))%>%
  mutate(name = paste(VERAS_Proposal_Number, temp))

# Base plot with the main heatmap
 ggplot(grade_summary, aes(x = Field, y = name, fill = score)) + 
  geom_tile(color = "white") +
...
what <- Truth%>%
  filter(VERAS_Proposal_Number == "V201060")%>%
  select(I._Indirect_Costs)
what2 <- Experiment%>%
  filter(VERAS_Proposal_Number == "V201060")%>%
  select(I._Indirect_Costs)
